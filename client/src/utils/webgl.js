...
export class InfiniteGridMenu {
    ...
    _initTexture() {
        ...
        this.items.forEach((item, i) => {
            ...
            }; img.onerror = checkLoad; img.src = item.cover || item.image; // Use cover if available
        });
    }
    ...
    _onControlUpdate(deltaTime) {
        ...
        if (isMoving !== this.movementActive) { this.movementActive = isMoving; if (this.onMovementChange) this.onMovementChange(isMoving); }
        if (!this.control.isPointerDown) {
            ...
            if (this.onActiveItemChange) this.onActiveItemChange(itemIndex); const snapDirection = vec3.normalize(vec3.create(), this._getVertexWorldPosition(nearestVertexIndex));
    ...
    run(time = 0) {
        this._deltaTime = Math.min(32, time - this._time); this._time = time; this._animate(this._deltaTime); this._render(); this.rafId = requestAnimationFrame(t => this.run(t));
    }
    destroy() {
        cancelAnimationFrame(this.rafId);
        // Add more cleanup if necessary (delete buffers, textures, etc)
    }
}
